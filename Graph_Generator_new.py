...
nNodes=100
nClique=10
maxCliqueSize=int(math.log(100,2) * math.log(100,2))
density=0.1
print("Max clique size", maxCliqueSize)
...
for i in range(len(clique_list)):
    print(len(clique_list[i])) 


######### save clique lists ###################

with open("ground_truth_clique_list_100_10_0.1.txt", "w") as gtcl:
...
for i in range(len(node_id)):
    if i+1 not in clique_nodes:
        for j in range(i+2, len(node_id)+1):
            x=[]
            x.append(i+1)
            x.append(j)
            edges.append(x)
...
with open("edge_file_100_10_0.1.txt", "w") as edgeid:
